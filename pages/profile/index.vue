      ...
      <div class="prof-header-wrap">
        <div class="prof-header">
          ...
          <div class="nickname-row">
            <h2>{{ userProfile.bio.nickname }}
              <router-link :to="{ name: 'update-profile', params: { id: id }}">
                <span class="edit-btn"><BIconPencilSquare scale="1.6"></BIconPencilSquare></span>
              </router-link>
            </h2>
          </div>
        </div>
...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    ...
    const { params: { id } } = to;
    this.fetchProfile(this.currentId);
    next();
  },
  methods: {
    async fetchProfile(id) {
       await this.$store.dispatch('users/FETCH_USER_PROFILE', { id })
        .then((response) => {
